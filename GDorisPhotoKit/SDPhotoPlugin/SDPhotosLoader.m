...
@interface SDPhotosLoaderOperation : NSObject <SDWebImageOperation>
...
@implementation SDPhotosLoaderOperation
...
@interface SDPhotosLoader ()

@property (nonatomic, strong, nonnull) NSHashTable<SDPhotosLoaderOperation *> *operationsTable;
@property (nonatomic, strong, nonnull) dispatch_queue_t fetchQueue;

@end

@implementation SDPhotosLoader
...
+ (SDPhotosLoader *)sharedLoader {
    static dispatch_once_t onceToken;
    static SDPhotosLoader *loader;
    dispatch_once(&onceToken, ^{
        loader = [[SDPhotosLoader alloc] init];
    });
    return loader;
}
...
- (instancetype)init {
    ...
    if (self) {
        ...
        self.fetchQueue = dispatch_queue_create("SDPhotosLoader", DISPATCH_QUEUE_SERIAL);
#if SD_UIKIT
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification {
    for (SDPhotosLoaderOperation *operation in self.operationsTable) {
        [operation cancel];
    }
...
- (id<SDWebImageOperation>)requestImageWithURL:(NSURL *)url options:(SDWebImageOptions)options context:(SDWebImageContext *)context progress:(SDImageLoaderProgressBlock)progressBlock completed:(SDImageLoaderCompletedBlock)completedBlock {
    ...
    SDPhotosLoaderOperation *operation = [[SDPhotosLoaderOperation alloc] init];
...
- (void)fetchImageWithAsset:(PHAsset *)asset operation:(SDPhotosLoaderOperation *)operation url:(NSURL *)url options:(SDWebImageOptions)options context:(SDWebImageContext *)context progress:(SDImageLoaderProgressBlock)progressBlock completed:(SDImageLoaderCompletedBlock)completedBlock {
...
- (void)fetchImageDataWithAsset:(PHAsset *)asset operation:(SDPhotosLoaderOperation *)operation url:(NSURL *)url options:(SDWebImageOptions)options context:(SDWebImageContext *)context progress:(SDImageLoaderProgressBlock)progressBlock completed:(SDImageLoaderCompletedBlock)completedBlock {
    PHImageRequestOptions *requestOptions;
    if ([context valueForKey:SDWebImageContextPhotosImageRequestOptions]) {
        requestOptions = [context valueForKey:SDWebImageContextPhotosImageRequestOptions];
    } else {
